...
const searchStickers = async(userId, query) => {
    ...
    const response = await fetch(searchStickerUrl, { headers: { apikey: apikey } });
    const data = await response.json();
    const { stickerList } = data.body;
    // lines 22 to 30 can probably be shortened using object.assign or smth else but idk how D:
    const stickers = stickerList.map(({ stickerId, packageId, artistName, stickerImg }) => [stickerId, packageId, artistName, stickerImg]);
